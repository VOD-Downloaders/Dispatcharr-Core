...
impl fmt::Display for RetrieveError
{
    fn fmt(&self, formatter: &mut fmt::Formatter) -> fmt::Result
    {
        match self
        {
            RetrieveError::FailedToSetupHTTP { error_type } =>
            {
                write!(formatter, "Failed to set up HTTP client with error: {}.", error_type)
            },
            RetrieveError::GETProviderInfoFailed { error_type } =>
            {
                write!(formatter, "Failed to retrieve episodes from Dispatcharr with error: {}.", error_type)
            },
            RetrieveError::ProviderInfoReturnedErrorStatus { status_code, error_type } =>
            {
                write!(
                    formatter,
                    "Failed to retrieve episodes from Dispatcharr with error code: {}. Additional context: {}.",
                    status_code.as_u16(),
                    error_type
                )
            },
            ...
            {
                write!(formatter, "Failed to parse episode response JSON, error: {}.", error_type)
            },
        }
    }
}
...
pub fn retrieve_episodes(options: &DownloadOptions) -> Result<(Seasons, M3UID), RetrieveError>
{
    ...
    let client = reqwest::blocking::Client::builder()
        ...
        .build()
        .map_err(|error| {
            return RetrieveError::FailedToSetupHTTP {
    ...
    let response: reqwest::blocking::Response = client.get(&url).header("X-Api-Key", options.api_key.as_str()).send().map_err(|error| {
        return RetrieveError::GETProviderInfoFailed {
            error_type: error.to_string(),
        };
    })?;
    ...
    let info = response.error_for_status().map_err(|error| {
        return RetrieveError::ProviderInfoReturnedErrorStatus {
            status_code: status,
            error_type: error.to_string(),
        };
    })?;
    let body = info.text().map_err(|_error| {
        return RetrieveError::ProviderInfoContainsNoBody;
    })?;

    trace!("Response body: {:?}", body);

    let json = serde_json::from_str::<ProviderInfoResponse>(body.as_str()).map_err(|error| {
        return RetrieveError::FailedToParseJSON {
            error_type: error.to_string(),
        };
    })?;
    ...
    for (_season_key, season_episodes) in json.episodes
    {
        for episode in season_episodes
        {
            seasons
                ...
                .or_insert(Season { episodes: Vec::new() })
                .episodes
                .push(Episode {
                    uuid: episode.uuid,
