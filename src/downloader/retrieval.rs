    ...
    fn fmt(&self, formatter: &mut fmt::Formatter) -> fmt::Result {
        match self {
            RetrieveError::FailedToSetupHTTP { error_type } => {
                write!(
                    formatter,
                    "Failed to set up HTTP client with error: {}.",
                    error_type
                )
            }
            RetrieveError::GETProviderInfoFailed { error_type } => {
                write!(
                    formatter,
                    "Failed to retrieve episodes from Dispatcharr with error: {}.",
                    error_type
                )
            }
            RetrieveError::ProviderInfoReturnedErrorStatus {
                status_code,
                error_type,
            } => {
                write!(formatter, "Failed to retrieve episodes from Dispatcharr with error code: {}. Additional context: {}.", status_code.as_u16(), error_type)
            }
            ...
            RetrieveError::FailedToParseJSON { error_type } => {
                write!(
                    formatter,
                    "Failed to parse episode response JSON, error: {}.",
                    error_type
                )
            }
        }
