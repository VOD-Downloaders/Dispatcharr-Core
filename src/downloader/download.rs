    ...
    fn fmt(&self, formatter: &mut fmt::Formatter) -> fmt::Result {
        match self {
            DownloadError::StartDownloadFailed { title, error_type } => {
                write!(
                    formatter,
                    "Starting download: \"{}\" failed with error: {}.",
                    title, error_type
                )
            }
            ...
            DownloadError::FailedToCreateFile {
                title,
                file,
                error_type,
            } => {
                write!(formatter, "Download: \"{}\" failed, because of being unable to create file \"{}\" due to error: {}.", title, file.display(), error_type)
            }
            DownloadError::FailedToCopyContentsToFile {
                title,
                file,
                error_type,
            } => {
                write!(formatter, "Download: \"{}\" failed, because of being unable to copy HTTP response contents to file \"{}\" with errorcode: {}.", title, file.display(), error_type)
            }
            DownloadError::ValidationFailed { title, error } => {
                write!(
                    formatter,
                    "Download: \"{}\" failed during validation, error: {}.",
                    title, error
                )
            }
        }
    ...
    fn fmt(&self, formatter: &mut fmt::Formatter) -> fmt::Result {
        match self {
            ValidationError::UnsupportedContainerType { container_type } => {
                write!(
                    formatter,
                    "Container type not supported for validating: {}.",
                    container_type
                )
            }
            ...
            ValidationError::FailedToGetFormat { error_type } => {
                write!(
                    formatter,
                    "Unable to read video format, error: {}.",
                    error_type
                )
            }
...
pub fn download_episode(
    ...
    let url = format!(
        "{}/proxy/vod/episode/{}?m3u_account_id={}",
        options.url, episode.uuid, m3u_id
    );
        ...
        match options.overwrite_mode {
            ...
            OverwriteMode::Bad => {
                if let Some(seconds) = episode.seconds {
                    let validation_result = validate_download(
                        output_file.as_path(),
...
fn download_attempt(
    ...
    if let Some(seconds) = expected_secs {
        ...
        warning!(
            "Unable to validate episode \"{}\", no duration retrieved from HTTP GET.",
            debug_title
        );
        Ok(())
    }
