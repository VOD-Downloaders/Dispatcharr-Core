...
pub enum DownloadError
{
    StartDownloadFailed
    {
        title: String, error_type: String
    },
    DownloadFailed
    {
        title: String, exit_code: i32
    },
    FailedToCreateFile
    {
        title: String, file: PathBuf, error_type: String
    },
    FailedToCopyContentsToFile
    {
        title: String, file: PathBuf, error_type: String
    },
    ValidationFailed
    {
        title: String, error: ValidationError
    },
}
...
impl fmt::Display for DownloadError
{
    fn fmt(&self, formatter: &mut fmt::Formatter) -> fmt::Result
    {
        match self
        {
            DownloadError::StartDownloadFailed { title, error_type } =>
            {
                write!(formatter, "Starting download: \"{}\" failed with error: {}.", title, error_type)
            },
            ...
            DownloadError::FailedToCreateFile { title, file, error_type } =>
            {
                write!(
                    formatter,
                    "Download: \"{}\" failed, because of being unable to create file \"{}\" due to error: {}.",
                    title,
                    file.display(),
                    error_type
                )
            },
            DownloadError::FailedToCopyContentsToFile { title, file, error_type } =>
            {
                write!(
                    formatter,
                    "Download: \"{}\" failed, because of being unable to copy HTTP response contents to file \"{}\" with errorcode: {}.",
                    title,
                    file.display(),
                    error_type
                )
            },
            DownloadError::ValidationFailed { title, error } =>
            {
                write!(formatter, "Download: \"{}\" failed during validation, error: {}.", title, error)
            },
        }
    }
}
...
impl fmt::Display for ValidationError
{
    fn fmt(&self, formatter: &mut fmt::Formatter) -> fmt::Result
    {
        match self
        {
            ...
            {
                write!(formatter, "Container type not supported for validating: {}.", container_type)
            },
            ValidationError::FailedToReadFile { error_type } =>
            {
                write!(formatter, "Read error: {}.", error_type)
            },
            ValidationError::FailedToGetFormat { error_type } =>
            {
                write!(formatter, "Unable to read video format, error: {}.", error_type)
            },
            ValidationError::NoTrackFound =>
            {
                write!(formatter, "Unable to find a video track in the file.")
            },
            ValidationError::FeatureNotFound { error_type } =>
            {
                write!(formatter, "Feature not found error: {}.", error_type)
            },
            ValidationError::PacketReadError { error_type } =>
            {
                write!(formatter, "Unable to read packets, error: {}.", error_type)
            },
...
pub fn download_episode(options: &DownloadOptions, episode: &Episode, m3u_id: M3UID) -> Result<(), DownloadError>
{
            ...
            OverwriteMode::Bad =>
            {
                if let Some(seconds) = episode.seconds
                {
                    ...
                    match validation_result
                    {
                        ...
                        {
                            info!(
                                "Episode \"{}\" already exists on disk, OverwriteMode::Bad selected, the episode has been fully validated, so skipping this episode.",
                                episode.title
                            );
                            return Ok(());
                        },
                        Err(error) =>
                        {
                            match error
                            {
                                ...
                                {
                                    warning!(
                                        "Episode \"{}\" already exists on disk, OverwriteMode::Bad selected, this episode cannot be validated because of missing feature: \"{}\", so skipping...",
                                        episode.title,
                                        error_type
                                    );
                                    return Ok(());
                                ...
                                {
                                    warning!(
                                        "Episode \"{}\" already exists on disk, OverwriteMode::Bad selected, this episode cannot be validated because of unsupported container type: \"{}\", so skipping...",
                                        episode.title,
                                        container_type
                                    );
                                    return Ok(());
                                ...
                                {
                                    warning!(
                                        "Episode \"{}\" already exists on disk, OverwriteMode::Bad selected, this episode failed validation with error: \"{}\", so overwriting.",
                                        episode.title,
                                        error
                                    );
                                },
    ...
    // Start attempts
    let mut last_error: DownloadError = DownloadError::StartDownloadFailed {
        title: "".to_string(),
...
) -> Result<(), DownloadError>
{
    let client = Client::builder()
        ...
        .build()
        .map_err(|e| DownloadError::StartDownloadFailed {
            title: debug_title.to_string(),
            error_type: e.to_string(),
        })?;

    let mut response = client.get(url).send().map_err(|e| DownloadError::StartDownloadFailed {
        title: debug_title.to_string(),
        error_type: e.to_string(),
    })?;
    ...
    let mut file = File::create(output_file).map_err(|e| DownloadError::FailedToCreateFile {
        title: debug_title.to_string(),
    ...
    if let Err(e) = response.copy_to(&mut file)
    {
        ...
        return Err(DownloadError::FailedToCopyContentsToFile {
            title: debug_title.to_string(),
            file: output_file.to_path_buf(),
            error_type: e.to_string(),
        });
        ...
        match validate_download(output_file, container_extension, seconds)
        {
            ...
            Err(error) =>
            {
                match error
                {
                    ...
                    {
                        warning!(
                            "Unable to validate episode \"{}\", this episode cannot be validated because of missing feature: \"{}\".",
                            debug_title,
                            error_type
                        );
                        return Ok(());
                    ...
                    {
                        warning!(
                            "Unable to validate episode \"{}\", this episode cannot be validated because of unsupported container type: \"{}\".",
                            debug_title,
                            container_type
                        );
                        return Ok(());
...
fn validate_download(output_file: &Path, container_extension: &str, expected_secs: u64) -> Result<u64, ValidationError>
{
    ...
    match container_extension
    {
        ...
        _ => Err(ValidationError::UnsupportedContainerType {
            container_type: container_extension.to_string(),
        }),
    }
}
...
{
    let file = File::open(path).map_err(|error| ValidationError::FailedToReadFile {
    ...
    let probed_result = symphonia::default::get_probe()
        ...
        .map_err(|error| ValidationError::FailedToGetFormat {
            error_type: error.to_string(),
        });
    ...
    if let Err(error) = &probed_result
    {
        if let ValidationError::FailedToGetFormat { error_type } = error
        {
            ...
            {
                return Err(ValidationError::FeatureNotFound {
    ...
    loop
    {
        match format.next_packet()
        {
            ...
            Err(error) =>
            {
                return Err(ValidationError::PacketReadError {
                    error_type: error.to_string(),
                });
            },
        }
